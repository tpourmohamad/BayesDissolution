...
#' @param N.gpq The number of generalized pivotal quantity samples.
...
#' f2gpq(dis_data, level = 0.9, N.gpq = 10000)
#'
#' @export
f2gpq <- function(dis_data, level = 0.9, N.gpq = 10000, ci.type = c("quantile", "HPD"), get.dist = FALSE){
 ...
 ##   i = 1,2 (groups); j=1, ..., P (time points), k = 1, 2, .., n (tablets).  Data must be balanced for this function!
  ...
  if(class(dis_data)[1] != "dis_data"){
  ...
  }else if(N.gpq <= 0 | !is.numeric(N.gpq)){
    stop("N.gpq must be a positive integer.")
    ...
    Z <- matrix(rnorm(N.gpq * dis_data$nTime), N.gpq, dis_data$nTime)
    U1 <- stats::rchisq(N.gpq, dis_data$degFreeGroupTab)
    U2 <- stats::rchisq(N.gpq, dis_data$degFreeRes)
    ...
    muDiff <- matrix(NA, N.gpq, dis_data$nTime)
    for (j in 1:N.gpq){
